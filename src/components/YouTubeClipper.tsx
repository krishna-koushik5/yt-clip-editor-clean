...
export default function YouTubeClipper({ onVideoLoad, onSaveClip }: YouTubeClipperProps) {
  ...
  const saveCurrentClip = async () => {
    ...
    setIsSaving(true);
    try {
      // Create the clip object with empty captions array for manual workflow
      const newClip: SavedClip = {
        ...
        originalUrl,
        createdAt: new Date(),
        captions: [], // Empty captions array for manual workflow
      };

      // Save the clip and show a success message
      if (onSaveClip) {
        onSaveClip(newClip);
      }
      ...
      toast.success(`Clip "${newClip.title}" saved successfully!`, {
        description: "Use the Subtitle Workflow panel to download MP3 and generate captions.",
      });
    } catch (error) {
